...
RSpec.describe "Delta dedup across runs", type: :integration do
  ...
  def run_aggregation(output_dir, cache_dir, released_dir)
    ...
    asset_processor = Metanorma::Release::AssetProcessor.new(output_dir: output_dir, routing: routing,
                                                             canonicalize: true)
    delta_state = Metanorma::Release::DeltaState.new(cache_store: Metanorma::Release::FileCacheStore.new(cache_dir),
  ...
  it "processes all releases on first run" do
    ...
    begin
      ...
      expect(result1.publications.length).to eq(2)
    ensure
  ...
  it "still returns same document count on second run" do
    ...
    begin
      ...
      expect(result2.publications.length).to eq(2)
    ensure
      FileUtils.rm_rf(output1)
