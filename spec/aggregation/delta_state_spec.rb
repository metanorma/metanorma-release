...
RSpec.describe Metanorma::Release::DeltaState do
  ...
  let(:state) do
    described_class.new(cache_store: cache, output_dir: output_dir)
  end
  ...
  describe "release dedup" do
    it "returns false for new release" do
      expect(state.processed?("repo/key", "cc-18011/ed1",
                              "hash123")).to be false
    end
    ...
    it "returns release files" do
      state.mark_processed("repo/key", "cc-18011/ed1", "hash",
                           %w[file.html file.pdf])
      expect(state.release_files("repo/key",
                                 "cc-18011/ed1")).to eq(%w[file.html file.pdf])
    end
