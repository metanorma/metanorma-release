...
RSpec.describe Metanorma::Release::PlatformFactory do
  describe '.build_publisher' do
    ...
    it "builds Local::Publisher for 'local' platform" do
      tmpdir = Dir.mktmpdir
      begin
        publisher = described_class.build_publisher('local', { output_dir: tmpdir })
    ...
    it 'raises ArgumentError for unknown platform' do
      expect { described_class.build_publisher('unknown', {}) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe described_class::NullManifestReader do
    it 'returns nil' do
      ...
      expect(reader.read(double('repo'))).to be_nil
    end
  end
end
